(function (root, factory) {
    if (typeof define === 'function' && define.amd) {
        ...
        root.WaveSurfer.MicrophoneStream = factory(root.WaveSurfer);
    }
}(this, function (WaveSurfer) {
    'use strict';

    WaveSurfer.MicrophoneStream = {
        init: function (params) {
            ...
            this.windowSize = this.params.windowSize || this.bufferSize * 100;
            this.inputBuffer = this.createBuffer(this.windowSize);
        },
        ...
        createBuffer: function (length) {
            return this.micContext.createBuffer(this.numberOfInputChannels, length, this.micContext.sampleRate);
        },
        /**
         * Redraw the waveform.
         */
        reloadBuffer: function(event) {
            if (!this.paused) {
                this.moveWindow(event.inputBuffer);
                this.wavesurfer.empty();
                this.wavesurfer.loadDecodedBuffer(this.inputBuffer);
            }
        },
        moveWindow: function(buffer) {
            var startOffset = this.inputBuffer.length - buffer.length;

            for (var i = 0; i < this.numberOfInputChannels; ++i) {
                var windowData = this.inputBuffer.getChannelData(i);
                windowData.set(windowData.subarray(buffer.length));
                windowData.set(buffer.getChannelData(i), startOffset);
            }
        },
        /**
    ...
    WaveSurfer.util.extend(WaveSurfer.MicrophoneStream, WaveSurfer.Observer);

    return WaveSurfer.MicrophoneStream;
}));
